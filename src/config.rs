    ...
    fn default() -> Self {
        ...
        providers.insert(
            ...
            ProviderConfig {
                ...
                models: vec!["qwen3.8-max".into(), "qwen3-coder-next".into(), "qwen3-coder-plus".into()],
            },
        );
...
pub fn load_or_create_config(path: &Path) -> RwLock<Config> {
    match fs::read_to_string(path) {
        Ok(content) => {
            ...
            // Migrate: add missing providers and reorder
            for (id, default_provider) in defaults.providers.iter() {
                if !cfg.providers.contains_key(id) {
                    cfg.providers.insert(id.clone(), default_provider.clone());
                    migrated = true;
                }
            }
            // Reorder to match defaults (Chinese → Foreign → Local → Custom)
            if migrated {
                let mut ordered = IndexMap::new();
                for (id, _) in defaults.providers.iter() {
                    if let Some(p) = cfg.providers.swap_remove(id) {
                        ordered.insert(id.clone(), p);
                    }
                }
                cfg.providers = ordered;
                if let Ok(toml_str) = toml::to_string_pretty(&cfg) {
                    let _ = fs::write(path, &toml_str);
                    info!("已迁移配置: 补充缺失的 provider 并重排");
                }
            }
